...
async function processTranslationDirs(translationDirs: string[], inputDir: string, outputDir: string) {
...
async function convertToJSON(tDirInputPath: string, inputFilename: string, tDirOutputPath: string) {
  const translationObject = await extractObjectDefinition(tDirInputPath, inputFilename);
  if (translationObject) {
    createJSONFile(translationObject, tDirOutputPath, inputFilename);
  }
}
function createJSONFile(translationObject: string, tDirOutputPath: string, inputFilename: string) {
  const translationJSON = fixKeys(translationObject);
  const translatonJSONWithoutSingle = removeSingleQuotes(translationJSON);
  fs.outputFile(path.join(tDirOutputPath, inputFilename.replace(/\.ts$/, '.json')), translatonJSONWithoutSingle);
}

function fixKeys(translationObject: string) {
  const objectKeysRegex = /(^\s+)(\w+)(:)/gm;
  const translationJSON = translationObject.replace(objectKeysRegex, '$1"$2"$3');
...
async function extractObjectDefinition(tDirInputPath: string, inputFilename: string) {
